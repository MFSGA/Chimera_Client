...
pub fn rule_routes(router: ThreadSafeRouter) -> Router<Arc<AppState>> {
...
async fn get_rule_providers(
    ...
    let mut providers = HashMap::new();
    for (name, provider) in state.router.get_rule_providers() {
        providers.insert(name.clone(), provider.as_map().await);
    }
    let mut res = HashMap::new();
...
async fn get_rule_provider(
    ...
    match state.router.get_rule_providers().get(&provider_name) {
        Some(provider) => {
            axum::response::Json(provider.as_map().await).into_response()
        }
        None => (
            StatusCode::NOT_FOUND,
...
async fn update_rule_provider(
    ...
    match state.router.get_rule_providers().get(&provider_name) {
        Some(provider) => match provider.update().await {
            Ok(()) => (StatusCode::ACCEPTED, "rule provider update started")
                .into_response(),
            Err(err) => (
                StatusCode::INTERNAL_SERVER_ERROR,
...
async fn get_rule_provider_rules(
    ...
    match state.router.get_rule_providers().get(&provider_name) {
        Some(provider) => {
            let rules = provider.list_rules(500).await;
            let mut res = HashMap::new();
...
async fn match_rule_provider(
    ...
    let Some(provider) = state.router.get_rule_providers().get(&provider_name)
    else {
        return (
            StatusCode::NOT_FOUND,
            format!("rule provider {provider_name} not found"),
