...
mod tests {
    ...
    #[tokio::test]
    async fn allocates_distinct_ports_and_reuses_released_port() {
        let pool = PortPool::new();
        let first = pool.next().await.expect("first virtual port");
        let second = pool.next().await.expect("second virtual port");

        assert_ne!(first, second);
        assert!(PORT_RANGE.contains(&first));
        assert!(PORT_RANGE.contains(&second));

        pool.release(first).await;

        let mut reused = false;
        for _ in 0..(MAX_PORT - MIN_PORT) {
            if pool.next().await.expect("virtual port") == first {
                reused = true;
                break;
            }
        }
        assert!(reused, "released port should return to the pool");
    }
}
