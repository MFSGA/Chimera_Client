    ...
    pub async fn new<P: AsRef<Path>>(
        ...
        if !path.as_ref().exists() || download_url.contains("force=true") {
            ...
            download(&download_url, path.as_ref(), &http_client)
                .await
                .map_err(|error| {
                    Error::InvalidConfig(format!("geosite download failed: {error}"))
                })?;
        }

        let bytes = tokio::fs::read(path).await?;
        let cache = geodata_proto::GeoSiteList::decode(bytes.as_slice()).map_err(
            |error| Error::InvalidConfig(format!("geosite decode failed: {error}")),
        )?;
        Ok(Self { cache })
    }
}
